...
### We want samples with no pCR and we will compare pre NAC with post NAC. Key 1 = 0,1
nacBL_samples = data.frame(fname=list.files(rccDir, pattern="*.RCC")) %>%
  ...
  dplyr::filter(key2 == 0 & (key1 == 1 | key1 == 0))
# 47 samples to analyse
...
rccSet <- newRccSet(
  rccFiles               = file.path(rccDir, nacBL_samples$fname)
  ...
  #,blankLabel             = "blank"
  ,experimentData.name    = "NAC_BL"
  ,experimentData.lab     = "LBMC"
  ,experimentData.contact = "amoyag@uma.es"
  ,experimentData.title   = "WGCNA BC_NAC effect PRE vs POST samples"
  ,experimentData.abstract= "Gene co-expression network to find genes associated with the effect of NAC in Basal-like samples."
)
...
nacBL_samples <- filter(nacBL_samples, fname %in% colnames(ncounts))
colnames(ncounts) <- nacBL_samples$sample
...
pdf(file = "Results/WGCNAplots/sampleClustering.pdf", width = 12, height = 9);
par(cex = 0.6);
par(mar = c(0,4,2,0))
...
clinDat <- nacBL_samples %>% dplyr::select(sample,key1)

names(clinDat) <- c("sample", "NAC")
clinDat = clinDat[keepSamples,]

datTrait <- dplyr::select(clinDat, NAC)
datTrait$NAC = as.numeric(datTrait$NAC)
rownames(datTrait) <- clinDat$sample
...
pdf("Results/WGCNAplots/dendogram.pdf",  width = 16, height = 9)
plotDendroAndColors(sampleTree2, traitColors,
                    groupLabels = names(datTrait),
                    main = "Sample dendrogram chemoresistance heatmap")
dev.off()
save(datExpr, datTrait, file = "Data/RData/NAC_BL-01-dataInput.RData")
...
# Plot the results:
par(mfrow = c(1,2));
pdf("Results/WGCNAplots/softpower_threshold.pdf",  width = 9, height = 9)
cex1 = 0.9;
...
pdf(file = 'Results/WGCNAplots/TOM-basedDiss_dendogram.pdf')
...
save(MEs, moduleLabels, moduleColors, geneTree, file = "Data/RData/NAC_BL-02-networkConstruction.RData")
...
load("Data/RData/NAC_BL-01-dataInput.RData")
load("Data/RData/NAC_BL-02-networkConstruction.RData")
...
# Module eigengene blue shows significant correlation with NAC (0.37 pval=0.02)
...
# Define variable NAC containing the NAC column of datTrait
nac = as.data.frame(datTrait$NAC);
names(nac) = "NAC"
...
geneTraitSignificance = as.data.frame(cor(datExpr, nac, use = "p"));
GSPvalue = as.data.frame(corPvalueStudent(as.matrix(geneTraitSignificance), nSamples));

names(geneTraitSignificance) = paste("GS.", names(nac), sep="");
names(GSPvalue) = paste("p.GS.", names(nac), sep="");
...
MM_vs_GS <- function(module, modNames, moduleColors){
  ...
  pdf(file = paste('Results/WGCNAplots/',module,'_MMvsGS.pdf'))
  par(mfrow = c(1,1));
...
MM_vs_GS(module = "blue", modNames, moduleColors)
...
modOrder = order(-abs(cor(MEs, nac, use = "p")));
...
geneOrder = order(-abs(geneInfo0$GS.NAC),geneInfo0$moduleColor);
...
# Separate modules
blue_module <- geneInfo %>% 
  dplyr::filter(moduleColor == "blue" & abs(MM.blue) >= 0.70 & abs(GS.NAC) >= 0.2) %>%
  dplyr::select(Accession, geneSymbol, entrezID, GS.NAC, p.GS.NAC, MM.blue, p.MM.blue)

module = "blue"
write.csv(blue_module, file = paste("Results/",module,"-genes.csv", sep=""), quote = F,row.names = F)

### Modules enrichment analysis  

module_enrichment_analysis <- function(geneInfo, module, GS=0.2, MM){
  
  MM.module <- paste("MM.",module,sep="")
  
  res <- geneInfo %>% 
    dplyr::filter(moduleColor == module & !is.na(entrezID) & abs(GS.NAC) >= GS & abs(geneInfo[,MM.module]) >= MM) %>%
    dplyr::select(Accession, geneSymbol, entrezID, GS.NAC) %>% 
    dplyr::mutate(entrezID = as.character(entrezID))
  
  genes <- res$entrezID
  
  enrich_rs <- enrich_cp(genes, comparison = "NAC")
...
module_enrichment_analysis(geneInfo, "blue", GS=0.2, MM = 0.7)
...
expand_module(pybin, diamond_path, "blue", network_path = "Interactomes/Ghiassian_interactome.tsv")
expand_module(pybin, diamond_path, "blue", network_path = "Interactomes/Metabolism-centered_interactome.tsv")
expand_module(pybin, diamond_path, "blue", network_path = "Interactomes/STRINGdb_interactome.tsv")
